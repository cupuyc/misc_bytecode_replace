...
public class ByteParser {
    ...
    public function readTag(originalBytes:ByteArray):SwfTag
    {
        ...
        var tagLengthExcludingRecordHeader:uint = tagCodeAndLength & 0x3f;
        if (tagLengthExcludingRecordHeader == 0x3f)
        {
            tagLengthExcludingRecordHeader = originalBytes.readUnsignedInt();
        }

        swfTag.tagBody = new ByteArray();
        if (tagLengthExcludingRecordHeader > 0)
        {
            originalBytes.readBytes(swfTag.tagBody, 0, tagLengthExcludingRecordHeader);
        }
